...
function addYoutubeToQueue(buttonClicked) {
  ...
  $("#queues").append("<li><button onclick=removeQueue(this.id) id='" + queueLength + "'> x </button><i class='fa fa-youtube'></i> <div class='queue-text'> <button onclick='queueClick(this.id)' id='" + ytID + "/|" + queueLength + "'>" + ytTitle + "</button> </div></li>");
}
...
function addSoundCloudToQueue(buttonClicked) {
  ...
  $("#queues").append("<li><button onclick=removeQueue(this.id) id='" + queueLength + "'> x </button><i class='fa fa-soundcloud'></i> <div class='queue-text'> <button onclick='queueClick(this.id)' id='" + scID + "/|" + queueLength + "'>" + scTitle + "</button> </div></li>");
}
  ...
  for (var i = 0; i < parseInt(localStorage.getItem("length")); ++i) {
    ...
    if (localStorage.getItem(strI) == 'undefined') {
      ...
      if (id.indexOf('soundcloud') == -1) {
        $("#queues").append("<li><button onclick=removeQueue(this.id) id='" + queueLength + "'> x </button><i class='fa fa-youtube'></i> <div class='queue-text'> <button onclick='queueClick(this.id)' id='" + id + "/|" + strI + "'>" + title + "</button> </div></li>");
      }
      else if (id.indexOf('soundcloud') > -1) {
        $("#queues").append("<li><button onclick=removeQueue(this.id) id='" + queueLength + "'> x </button><i class='fa fa-soundcloud'></i> <div class='queue-text'> <button onclick='queueClick(this.id)' id='" + id + "/|" + strI + "'>" + title + "</button> </div></li>");
      }
      else {
